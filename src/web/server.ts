...
import Busboy from "busboy";              // いま未使用（将来のファイル受信で使えます）
...
// 通話XP 設定（★ +1XP は環境変数いらずで既定1）
const CALL_TOTALIZE_5MIN = String(process.env.CALL_TOTALIZE_5MIN || "1") === "1";
const CALL_XP_PER_CALL = (process.env.CALL_XP_PER_CALL === undefined || process.env.CALL_XP_PER_CALL === "") ? 1 : Number(process.env.CALL_XP_PER_CALL);
...
function buildHabiticaMap(s: string) {
  ...
  for (const [k, v] of Object.entries(p)) { if (v?.userId && v?.apiToken) out[k.toLowerCase()] = { userId: String(v.userId), apiToken: String(v.apiToken) }; }
  return out;
}
function buildNameEmailMap(s: string) {
  const p = safeParse<Record<string, string>>(s) || {};
  const out: Record<string, string> = {};
  for (const [n, e] of Object.entries(p)) { if (!n || !e) continue; out[normSpace(n)] = e.toLowerCase(); }
  return out;
}
function buildZoomEmailMap(s: string) {
  const p = safeParse<Record<string, string>>(s) || {};
  const out: Record<string, string> = {};
  for (const [z, e] of Object.entries(p)) { if (!z || !e) continue; out[z] = e.toLowerCase(); }
  return out;
}
...
app.get("/healthz", (_req, res) => {
  res.json({
    ok: true,
    version: "2025-09-18-outbound-only",
...
// =============== Zoom Webhook（ts+base64 / HEX 両対応） ===============
function readBearerFromHeaders(req: Request) {
  for (const k of ["authorization", "x-authorization", "x-auth", "x-zoom-authorization", "zoom-authorization"]) {
    const v = req.get(k); if (!v) continue; const m = v.trim().match(/^Bearer\s+(.+)$/i); return (m ? m[1] : v).trim();
  }
  return "";
}
...
// 方向（発信/着信）判定
function detectDirection(raw: any, evType?: string): "outbound" | "inbound" | "unknown" {
  const d = String(
    raw?.direction ||
    raw?.call_direction ||
    raw?.callLog?.direction ||
    raw?.call_logs?.[0]?.direction ||
    ""
  ).toLowerCase();
  ...
  const et = String(evType || "").toLowerCase();
  if (et.includes("caller_call_history_completed")) return "outbound";
  if (et.includes("callee_call_history_completed")) return "inbound";

  return "unknown";
}

// Zoom から duration を堅牢に取得
function zoomDurationMs(raw: any): number {
  // 代表的な候補（ms値 / 秒値 / start-end）
  const tryNums = (...vals: any[]) => vals.map(Number).find(v => Number.isFinite(v) && v > 0);
  const talkSec =
    tryNums(raw?.talk_time, raw?.call_log?.talk_time, raw?.call_logs?.[0]?.talk_time);
  const msCandidate = tryNums(
    raw?.duration_ms, raw?.call_duration_ms, raw?.durationMs, raw?.duration, raw?.call_duration
  );
  let ms = 0;
  if (msCandidate) ms = msCandidate >= 100000 ? Math.floor(msCandidate) : Math.floor(msCandidate * 1000);
  if (!ms && talkSec) ms = Math.floor(Number(talkSec) * 1000);

  if (!ms && raw?.start_time && raw?.end_time) {
...
app.post("/webhooks/zoom", async (req: Request & { rawBody?: Buffer }, res: Response) => {
  ...
  const evType = String(b?.event || b?.payload?.event || raw?.event || "").trim();
  ...
  // 方向（発信/着信）
  const direction = detectDirection(raw, evType);
  raw.direction = direction;

  // XPターゲットは発信者のみ。caller 以外は即スキップ（記録も不要ならここで返す）
  const isCallerEvent =
    /phone\.caller_call_history_completed/i.test(evType) ||
    /caller call history completed/i.test(evType);
  const isAiSummary =
    /phone\.ai_call_summary_changed/i.test(evType) ||
    /call summary changed/i.test(evType);

  // Summary はXP付与しない（今は無視）
  if (!isCallerEvent && !isAiSummary) return res.json({ ok: true, skipped: "non-caller event" });

  // duration
  const ms = zoomDurationMs(raw);
  const callId =
    raw.call_id || raw.session_id || raw.callID || raw.sessionID || b.id || `zoom:${Date.now()}`;

  log(`[zoom] accepted event=${evType} callId=${callId} ms=${ms} dir=${direction}`);
  ...
  res.json({ ok: true, accepted: true, ms, direction, evType });
});
...
function resolveActor(ev: { source: "v3" | "workflow"; raw?: any }): { name: string; email?: string } {
  ...
  let email: string | undefined =
    ...
    raw?.owner?.email || raw?.properties?.hs_created_by_user_id?.email || raw?.userEmail;
...
async function awardXpForAppointment(ev: Normalized) {
  ...
  if (!cred || DRY_RUN) {
    log(`[XP] appointment (DRY_RUN or no-cred) callId=${ev.callId} by=${who.name} @${when}`);
...
async function awardXpForCallDuration(ev: CallDurEv) {
  ...
  // 方向（発信のみ XP 付与）
  const dir = String(ev?.raw?.direction || "").toLowerCase();
  const isOutbound = dir === "outbound" || dir === "caller";

  // ログ保存（ダッシュボード用）— 着信も見えるように dir を保持
  ...
  if (!isOutbound) {
    log(`[call] inbound (no XP) by=${who.name} @${when}`);
    return;
  }
  ...
  if (CALL_XP_PER_CALL > 0) {
    const cred = getHabitica(who.email);
    if (!cred || DRY_RUN) {
      log(`[call] per-call +${CALL_XP_PER_CALL}XP (DRY_RUN or no-cred) by=${who.name} @${when}`);
    } else {
  ...
  if (CALL_TOTALIZE_5MIN) {
    ...
    try {
      ...
      console.error("[call-totalize] habitica failed:", e?.message || e);
    }
...
async function handleCallDurationEvent(ev: CallDurEv) {
  ...
  if (hasSeen(id)) return; markSeen(id);
  if (ev.durationMs <= 0 && CALL_XP_PER_CALL <= 0) return; // 0秒でも +1XP は付与するので、ベースが0の時だけ弾く
...
app.get("/admin/dashboard", (_req, res) => {
  ...
  const calls = rd("data/events/calls.jsonl");        // Zoom/HubSpotからの通話
  ...
  function agg(day: string) {
    ...
    // ★ 発信のみで集計
    for (const x of calls.filter(v => v.day === day && v.dir !== 'inbound')) {
      const k = nm(x); by[k] ??= { name: k, calls: 0, min: 0, appts: 0, apprs: 0, sales: 0 };
      by[k].calls += 1; by[k].min += Math.round((x.ms || 0) / 60000);
    }
