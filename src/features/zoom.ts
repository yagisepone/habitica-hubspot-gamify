...
import { AUTH_TOKEN, ZOOM_BEARER_TOKEN, ZOOM_SIG_SKEW, ZOOM_VERIFICATION_TOKEN, ZOOM_WEBHOOK_SECRET } from "../lib/env.js";
...
function pickZoomInfo(obj: any) {
  const o = obj || {};
  const logs: any[] = Array.isArray(o.call_logs) ? o.call_logs : Array.isArray(o?.object?.call_logs) ? o.object.call_logs : [];
  const chosen = logs.find((x) => String(x?.direction || "").toLowerCase() === "outbound") || logs[0] || o;

  const emailRaw = o.user_email || o.owner_email || o.caller_email || o.callee_email || chosen?.caller_email || chosen?.callee_email || "";
  const email = String(emailRaw || "").toLowerCase() || undefined;

  const zid = o.zoom_user_id || o.user_id || o.owner_id || chosen?.zoom_user_id || chosen?.user_id || chosen?.owner_id || undefined;
  ...
  if (typeof talkSecCand === "number" && isFinite(talkSecCand)) {
    ...
    if (Number.isFinite(st) && Number.isFinite(et)) ms = Math.max(0, et - st); else ms = 0;
  }
  if (ms < 0) ms = 0;
  if (ms > 3 * 60 * 60 * 1000) ms = 3 * 60 * 60 * 1000; // 3h guard

  const callId = o.call_id || o.session_id || chosen?.call_id || chosen?.session_id || `zoom:${Date.now()}`;
...
export async function zoomWebhook(req: Request & { rawBody?: Buffer }, res: Response) {
  ...
  if (String(info.dir) === "inbound") {
    const when = fmtJST(b.timestamp || info.endedAt || Date.now());
